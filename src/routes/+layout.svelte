<script lang="ts">
	...
	function checkAuthStatus(): void {
		...
		if (isAuthenticated && userType) {
			...
			// If on public pages, redirect to appropriate dashboard
	...
	// Reactive statement to update auth status
	$: {
...
<div class="min-h-screen bg-gray-50">
	<!-- Navigation for public pages only -->
	{#if !isAuthenticated || (!$page.url.pathname.startsWith('/admin') && !$page.url.pathname.startsWith('/customer'))}
					...
					<div class="hidden items-center space-x-8 md:flex">
						{#if isAuthenticated}
							...
							<span class="text-sm text-gray-600">
								Selamat datang, {userType === 'admin' ? 'Admin' : 'Customer'}
							</span>
							<a
								href={userType === 'admin' ? '/admin' : '/customer'}
								class="rounded-md px-3 py-2 text-sm font-medium text-primary-600 hover:text-primary-700"
							>
								Dashboard
							</a>
							<button
								on:click={handleLogout}
								class="rounded-md px-3 py-2 text-sm font-medium text-gray-700 hover:text-red-600"
							>
								Logout
							</button>
						{:else}
					...
					<div class="space-y-1 border-t bg-white px-2 pt-2 pb-3 sm:px-3">
						{#if isAuthenticated}
							...
							<button
								...
								class="block w-full rounded-md px-3 py-2 text-left text-base font-medium text-red-600 hover:bg-red-50"
							>
								Logout
							</button>
						{:else}
					...
					<div>
						...
						<div class="space-y-3">
							...
							<a href="/about" class="block text-gray-300 transition-colors hover:text-white"
								>Tentang</a
							>
							<a href="/tarif" class="block text-gray-300 transition-colors hover:text-white"
								>Tarif</a
							>
							{#if !isAuthenticated}
								<a href="/login" class="block text-gray-300 transition-colors hover:text-white"
									>Login</a
								>
							{/if}
						</div>
